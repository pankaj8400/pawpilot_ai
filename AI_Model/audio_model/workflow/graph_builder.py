...
from AI_Model.audio_model.workflow.nodes_audio import (
...
from AI_Model.audio_model.workflow.workflow_state import WorkflowState as AudioWorkflowState , create_audio_state
...
class StateTransformer:
    ...
    @staticmethod
    def audio_to_workflow(audio_state: AudioWorkflowState) -> WorkFlowState:
        """
        Transform AudioWorkflowState output to WorkFlowState input.
        
        Maps audio model outputs to workflow state fields.
        """
        try:
            logger.info("Transforming AudioWorkflowState to WorkFlowState")
            
            workflow_state = create_workflow_state(
                query=audio_state.get("query", ""),
                user_id=audio_state.get("user_id", "user_1"),
                session_id=audio_state.get("session_id", ""),
                use_rag=audio_state.get("use_rag", True),
                model_to_use=audio_state.get("model_to_use", "gpt-4-turbo"),
                strategy=audio_state.get("strategy", "emotion-detection-audio")
            )
            # Map audio-specific outputs to workflow inputs
            workflow_state["context"] = f"""
Audio Model Analysis:
- Predicted Class: {audio_state.get("response", "Unknown")}
- Confidence Score: {audio_state.get("confidence_score", 1.0):.2f}
- meta data retrieved: {audio_state.get("retrieved_docs", [])}
"""
            workflow_state["predicted_class"] = str(audio_state.get("response", "Unknown"))
            # Carry over retrieved docs if any
            if audio_state.get("retrieved_docs"):
                workflow_state["retrieved_docs"] = [
                    {"content": str(doc)} for doc in audio_state.get("retrieved_docs", {}).values()
                ]
            
            # Use audio model's inference time
            if audio_state.get("inference_time"):
                workflow_state["inference_time"] = audio_state.get("inference_time", 0.0)
            
            # Preserve timing info
            workflow_state["start_time"] = audio_state.get("start_time", time.time())
            
            # Carry forward any errors
            if audio_state.get("error"):
                workflow_state["error"] = audio_state.get("error", [])
    ...
    def workflow_to_final_output(workflow_state: WorkFlowState) -> dict:
        ...
        try:
            ...
            return {
                ...
                "audio_output": workflow_state.get("context", ""),
...
class MultiGraphWorkflow:
    ...
    def build_subgraph_a(self) -> CompiledStateGraph:
        ...
        """
        try:
            logger.info("Building Subgraph A (Audio Processing)")
            
            graph_a = StateGraph(AudioWorkflowState)
            
            # Add nodes (01-05)
            graph_a.add_node("input_processing", input_processing_node)      # 01
            graph_a.add_node("emotion_detection", emotion_detection_node)    # 02
            graph_a.add_node("retrieval", retrieval_node)                    # 05
            
            # Define edges for subgraph A
            graph_a.add_edge(START, "input_processing")
            graph_a.add_edge("input_processing", "emotion_detection")
            graph_a.add_edge("emotion_detection", "retrieval")
            graph_a.add_edge("retrieval", END)
    ...
    @staticmethod
    def _route_decision_a(state: AudioWorkflowState) -> str:
    ...
    def invoke(self, input_data: dict):
        ...
        try:
            ...
            audio_inputs = input_data.get("audio_file")
            if audio_inputs is None:
                audio_inputs = input_data.get("audio_files", [])
            
            # Create initial audio state
            audio_state = create_audio_state(
                audio_file = audio_inputs,
            ...
            if "audio_file" in input_data:
                audio_state["audio_file"] = input_data["audio_file"]
            elif "audio_files" in input_data:
                audio_state["audio_file"] = input_data["audio_files"]
            ...
            # Execute audio subgraph FIRST with audio_state
            audio_workflow = self.subgraph_a
            assert audio_workflow is not None, "Subgraph A failed to build"
            audio_result = audio_workflow.invoke(audio_state)
            
            logger.info(f"Audio subgraph completed. Predicted class: {audio_result.get('predicted_class')}")
            
            # Transform audio result to workflow state
            workflow_state = self.state_transformer.audio_to_workflow(cast(AudioWorkflowState, audio_result))
    ...
    def stream(self, input_data: dict):
        ...
        try:
            ...
            # Stream audio subgraph first
            for event in self.subgraph_a.stream(audio_state):
                logger.debug(f"Audio stream event: {event}")
                yield {"stage": "audio", "event": event}
            
            # Get final audio result
            audio_result = self.subgraph_a.invoke(audio_state)
...
if __name__ == "__main__":
    try:
        ...
        import numpy as np
        initial_input = {
            "audio_file": ["AI_Model/audio_model/data/dog_barking_13.wav"],  # Simulated 1-second audio at 16kHz
            "user_id": "user_123",
